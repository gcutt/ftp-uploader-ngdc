...
def uploadfiles(local_paths, folder_name, progress_callback=None, file_done_callback=None):
                ...
                try:
                    with open(path, "rb") as file:
                        ...
                        def handle_chunk(chunk):
                            nonlocal file_bytes, total_transferred
                            file_bytes += len(chunk)
                            total_transferred += len(chunk)
                            if progress_callback:
                                progress_callback(filename, file_bytes, total_transferred)
                        ftp.storbinary(f"STOR {filename}", file, 8192, callback=handle_chunk)
                        print(f"🚀 Uploaded {filename}")
                        results.append(f"✅ {filename}")
                        if file_done_callback:
                            file_done_callback(filename, success=True)
                except Exception as e:
                    print(f"❌ Failed to upload {filename}: \n{e}")
                    results.append(f"❌ {filename}: \n{e}")
                    if file_done_callback:
                        file_done_callback(filename, success=False)
